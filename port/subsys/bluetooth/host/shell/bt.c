...
static int cmd_scan_off(const struct shell *sh, uint8_t dev_id);
...
static void scan_recv(const struct bt_le_scan_recv_info *info, struct net_buf_simple *buf)
{
	...
	shell_print(ctx_shell, "dev:%d, %s%s, AD evt type %u, RSSI %i %s "
	...
	if ((info->adv_props & BT_GAP_ADV_PROP_CONNECTABLE) != 0U) {
		struct bt_conn *conn = bt_conn_lookup_addr_le_mc(info->dev_id, selected_id, info->addr);
		...
		if (conn == NULL) {
			...
			if (auto_connect.connect_name) {
				...
				cmd_scan_off(ctx_shell, info->dev_id);
...
static void bt_ready(uint8_t dev_id, int err)
{
	...
	shell_print(ctx_shell, "Bluetooth dev: %d initialized", dev_id);
...
#if defined(CONFIG_BT_OBSERVER)
	bt_le_scan_cb_register_mc(dev_id, &scan_callbacks);
#endif
	...
	bt_conn_cb_unregister_mc(dev_id, &conn_callbacks);
	bt_conn_cb_register_mc(dev_id, &conn_callbacks);
#endif /* CONFIG_BT_CONN */

#if defined(CONFIG_BT_PER_ADV_SYNC)
	bt_le_per_adv_sync_cb_register_mc(dev_id, &per_adv_sync_cb);
#endif /* CONFIG_BT_PER_ADV_SYNC */

#if defined(CONFIG_BT_SMP)
	bt_conn_auth_info_cb_register_mc(dev_id, &auth_info_cb);
#endif /* CONFIG_BT_SMP */
}
...
static int cmd_name(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc == 2) {
		shell_print(sh, "Bluetooth: %d Local Name: %s", dev_id, bt_get_name_mc(dev_id));
		return 0;
	}

	err = bt_set_name_mc(dev_id, argv[2]);
	if (err) {
		shell_error(sh, "Unable to set hci:%d name %s (err %d)", dev_id, argv[1],
			    err);
		return err;
	}
...
static int cmd_appearance(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc == 2) {
		shell_print(sh, "Bluetooth: %d Appearance: 0x%04x", dev_id, bt_get_appearance_mc(dev_id));
		return 0;
	}
	...
	err = bt_set_appearance_mc(dev_id, app);
	if (err) {
...
static int cmd_id_show(const struct shell *sh, size_t argc, char *argv[])
{
	...
	dev_id = strtoul(argv[1], NULL, 10);
	bt_id_get_mc(dev_id, addrs, &count);
...
#if defined(CONFIG_BT_OBSERVER)
static int cmd_active_scan_on(const struct shell *sh, uint8_t dev_id, uint32_t options,
			      uint16_t timeout)
{
	int err;
	...
	err = bt_le_scan_start_mc(dev_id, &param, NULL);
	if (err) {
...
static int cmd_passive_scan_on(const struct shell *sh, uint8_t dev_id, uint32_t options,
			       uint16_t timeout)
{
	struct bt_le_scan_param param = {
...
static int cmd_scan(const struct shell *sh, size_t argc, char *argv[])
{
	...
	action = argv[2];
	if (!strcmp(action, "on")) {
		return cmd_active_scan_on(sh, dev_id, options, timeout);
	} else if (!strcmp(action, "off")) {
		return cmd_scan_off(sh, dev_id);
	} else if (!strcmp(action, "passive")) {
		return cmd_passive_scan_on(sh, dev_id, options, timeout);
	} else {
		shell_help(sh);
		return SHELL_CMD_HELP_PRINTED;
	}
...
void set_ad_device_name_complete(struct bt_data *ad, uint8_t dev_id)
{
	const char *name = bt_get_name_mc(dev_id);
...
static int cmd_advertise(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (name_ad && with_name) {
		set_ad_device_name_complete(&ad[0], dev_id);
		ad_len++;
	}

	if (name_sd && with_name) {
		set_ad_device_name_complete(&sd[0], dev_id);
		sd_len++;
	}
	...
	err = bt_le_adv_start_mc(dev_id, &param, ad_len > 0 ? ad : NULL, ad_len, sd_len > 0 ? sd : NULL,
			      sd_len);
	if (err < 0) {
			     ...
			     size_t argc, char *argv[])
{
	...
	err = bt_addr_le_from_str(argv[2], argv[3], &addr);
	...
	err = bt_le_adv_start_mc(dev_id, &param, NULL, 0, NULL, 0);
	if (err) {
...
static int cmd_adv_create(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_le_ext_adv_create_mc(dev_id, &param, &adv_callbacks, &adv);
	if (err) {
...
static int cmd_adv_data(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (name && dev_name && name_value == NULL) {
		...
		set_ad_device_name_complete(&data[*data_len], dev_id);
...
static int cmd_connect_le(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_conn_le_create_mc(dev_id, &addr, create_params, BT_LE_CONN_PARAM_DEFAULT,
				&conn);
	if (err) {
...
static int cmd_disconnect(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (default_conn && argc < 4) {
		...
		conn = bt_conn_lookup_addr_le_mc(dev_id, selected_id, &addr);
	}
...
static int cmd_oob(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_le_oob_get_local_mc(0, selected_id, &oob_local);
	if (err) {
		shell_error(sh, "OOB data failed");
		return err;
	}
...
static int cmd_clear(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (strcmp(argv[2], "all") == 0) {
		err = bt_unpair_mc(dev_id, selected_id, NULL);
		if (err) {
...
#if defined(CONFIG_BT_CLASSIC)
		addr.type = BT_ADDR_LE_PUBLIC;
		err = bt_addr_from_str(argv[2], &addr.a);
#else
	...
	err = bt_unpair_mc(dev_id, selected_id, &addr);
	if (err) {
...
static int cmd_bonds(const struct shell *sh, size_t argc, char *argv[])
{
	...
	bt_foreach_bond_mc(dev_id, selected_id, bond_info, &bond_count);
...
static int cmd_connections(const struct shell *sh, size_t argc, char *argv[])
{
	...
	bt_conn_foreach_mc(dev_id, BT_CONN_TYPE_ALL, connection_info, &conn_count);
...
static int cmd_auth(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (!strcmp(argv[2], "all")) {
		err = bt_conn_auth_cb_register_mc(dev_id, &auth_cb_all);
	} else if (!strcmp(argv[2], "input")) {
		err = bt_conn_auth_cb_register_mc(dev_id, &auth_cb_input);
	} else if (!strcmp(argv[2], "display")) {
		err = bt_conn_auth_cb_register_mc(dev_id, &auth_cb_display);
	} else if (!strcmp(argv[2], "yesno")) {
		err = bt_conn_auth_cb_register_mc(dev_id, &auth_cb_display_yes_no);
	} else if (!strcmp(argv[2], "confirm")) {
		err = bt_conn_auth_cb_register_mc(dev_id, &auth_cb_confirm);
	} else if (!strcmp(argv[2], "oob")) {
		err = bt_conn_auth_cb_register_mc(dev_id, &auth_cb_oob);
	} else if (!strcmp(argv[2], "status")) {
		err = bt_conn_auth_cb_register_mc(dev_id, &auth_cb_status);
	} else if (!strcmp(argv[2], "none")) {
		err = bt_conn_auth_cb_register_mc(dev_id, NULL);
	} else {
		shell_help(sh);
		return SHELL_CMD_HELP_PRINTED;
	}
				    ...
				    size_t argc, char *argv[])
{
	...
	if (argc > 2) {
		...
		err = bt_addr_from_str(argv[2], &addr);
		if (err) {
			shell_print(sh, "Invalid peer address (err %d)", err);
			return -ENOEXEC;
		}
	
		conn = bt_conn_lookup_addr_br_mc(dev_id, &addr);
		if (!conn) {
			shell_print(sh, "Not connected");
			return -ENOEXEC;
		}
				    ...
				    size_t argc, char *argv[])
{
	...
	if (argc > 2) {
		...
		return bt_conn_auth_pairing_confirm(conn);
	}
			    ...
			    size_t argc, char *argv[])
{
	...
	if (argc > 3) {
		...
		err = bt_addr_from_str(argv[3], &addr);
		if (err) {
			shell_print(sh, "Invalid peer address (err %d)", err);
			return -ENOEXEC;
		}
				   ...
				   size_t argc, char *argv[])
{
	...
	err = 0;
	type = shell_strtoul(argv[2], 0, &err);
	if (err ||
