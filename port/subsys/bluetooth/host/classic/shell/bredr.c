...
static int cmd_connect(const struct shell *sh, size_t argc, char *argv[])
{
	...
	conn = bt_conn_create_br_mc(dev_id, &addr, BT_BR_CONN_PARAM_DEFAULT);
	if (!conn) {
...
static void br_discovery_recv(const struct bt_br_discovery_result *result)
{
	br_device_found(&result->addr, result->rssi, result->cod, result->eir);
}
...
static int cmd_discovery(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (!strcmp(action, "on")) {
		...
		if (argc > 3) {
			param.length = atoi(argv[3]);
		}

		if (argc > 4 && !strcmp(argv[4], "limited")) {
			param.limited = true;
		}

		bt_br_discovery_cb_register_mc(dev_id, &discovery_cb);

		if (bt_br_discovery_start_mc(dev_id, &param, br_discovery_results,
		...
		if (bt_br_discovery_stop_mc(dev_id)) {
			shell_print(sh, "Failed to stop discovery");
			return -ENOEXEC;
		}
		bt_br_discovery_cb_unregister_mc(dev_id, &discovery_cb);
			      ...
			      size_t argc, char *argv[])
{
	...
	br_server.psm = strtoul(argv[2], NULL, 16);

	if (bt_l2cap_br_server_register_mc(dev_id, &br_server) < 0) {
			    ...
			    size_t argc, char *argv[])
{
	...
	if (!strcmp(action, "on")) {
		err = bt_br_set_discoverable_mc(dev_id, true);
	} else if (!strcmp(action, "off")) {
		err = bt_br_set_discoverable_mc(dev_id, false);
	} else {
		shell_help(sh);
		return SHELL_CMD_HELP_PRINTED;
	}
			   ...
			   size_t argc, char *argv[])
{
	...
	if (!strcmp(action, "on")) {
		err = bt_br_set_connectable_mc(dev_id, true);
	} else if (!strcmp(action, "off")) {
		err = bt_br_set_connectable_mc(dev_id, false);
	} else {
		shell_help(sh);
		return SHELL_CMD_HELP_PRINTED;
	}
...
static int cmd_oob(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_br_oob_get_local_mc(0, &oob);
	if (err) {
