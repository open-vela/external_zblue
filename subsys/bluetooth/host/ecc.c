...
static void generate_pub_key(struct k_work *work)
{
	struct bt_ecc *ecc = CONTAINER_OF(work, struct bt_ecc, pub_key_work);
	...
	sys_memcpy_swap(hdev->pub_key, ecc->public_key_be, BT_PUB_KEY_COORD_LEN);
	sys_memcpy_swap(&hdev->pub_key[BT_PUB_KEY_COORD_LEN],
	...
	atomic_clear_bit(ecc->flags, PENDING_PUB_KEY);
...
static void generate_dh_key(struct k_work *work)
{
	struct bt_ecc *ecc = CONTAINER_OF(work, struct bt_ecc, dh_key_work);
	struct bt_dev *hdev = CONTAINER_OF(ecc, struct bt_dev, ecc);
...
int bt_pub_key_gen(struct bt_dev *hdev, struct bt_pub_key_cb *new_cb)
{
	...
	if (atomic_test_and_set_bit(hdev->ecc.flags, PENDING_PUB_KEY)) {
		return 0;
	}
...
int bt_dh_key_gen(struct bt_dev *hdev, const uint8_t remote_pk[BT_PUB_KEY_LEN], bt_dh_key_cb_t cb)
{
	...
	if (hdev->dh_key_cb ||
	    atomic_test_bit(hdev->ecc.flags, PENDING_PUB_KEY) ||
	    atomic_test_and_set_bit(hdev->ecc.flags, PENDING_DHKEY)) {
		return -EBUSY;
	}
	 ...
	 */
	sys_memcpy_swap(hdev->ecc.public_key_be, remote_pk, BT_PUB_KEY_COORD_LEN);
