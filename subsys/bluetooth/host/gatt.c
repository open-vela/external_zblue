...
static struct gatt_sc_cfg *find_sc_cfg(struct bt_dev *hdev, uint8_t id, const bt_addr_le_t *addr)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(hdev->gatt->sc_cfg); i++) {
		if (id == hdev->gatt->sc_cfg[i].id &&
		    bt_addr_le_eq(&hdev->gatt->sc_cfg[i].peer, addr)) {
			return &hdev->gatt->sc_cfg[i];
		}
	}
...
static void set_all_change_unaware(void)
{
	 ...
	 *   right away.
	 */
	for (size_t i = 0; i < ARRAY_SIZE(hdev->gatt->cf_cfg); i++) {
		struct gatt_cf_cfg *cfg = &hdev->gatt->cf_cfg[i];
...
static struct gatt_cf_cfg *find_cf_cfg(struct bt_conn *conn)
{
	...
	for (i = 0; i < ARRAY_SIZE(hdev->gatt->cf_cfg); i++) {
		struct gatt_cf_cfg *cfg = &hdev->gatt->cf_cfg[i];
...
static int db_hash_finish(struct gen_hash_state *state)
{
	...
	if (psa_mac_sign_finish(&(state->operation), hdev->gatt->db_hash.hash, 16,
...
static int db_hash_finish(struct gen_hash_state *state)
{
	if (tc_cmac_final(hdev->gatt->db_hash.hash, &(state->state)) == TC_CRYPTO_FAIL) {
		LOG_ERR("CMAC finish failed");
		return -EIO;
	}
	return 0;
...
static void db_hash_store(void)
{
	...
	err = bt_settings_store_hash(&hdev->gatt->db_hash.hash, sizeof(hdev->gatt->db_hash.hash));
	if (err) {
...
static void db_hash_gen(void)
{
	 ...
	 */
	sys_mem_swap(hdev->gatt->db_hash.hash, sizeof(hdev->gatt->db_hash.hash));

	LOG_HEXDUMP_DBG(hdev->gatt->db_hash.hash, sizeof(hdev->gatt->db_hash.hash), "Hash: ");

	atomic_set_bit(hdev->gatt->gatt_sc.flags, DB_HASH_VALID);
}
...
{
	bool new_hash = !atomic_test_bit(hdev->gatt->gatt_sc.flags, DB_HASH_VALID);
	...
	bool hash_loaded_from_settings =
		atomic_test_bit(hdev->gatt->gatt_sc.flags, DB_HASH_LOAD);
	bool already_processed =
		atomic_test_bit(hdev->gatt->gatt_sc.flags, DB_HASH_LOAD_PROC);
	...
	if (already_processed) {
		 ...
		 */
		atomic_set_bit(hdev->gatt->gatt_sc.flags, DB_HASH_LOAD_PROC);

		/* Check if hash matches then skip SC update */
		if (!memcmp(hdev->gatt->db_hash.stored_hash, hdev->gatt->db_hash.hash,
			    sizeof(hdev->gatt->db_hash.stored_hash))) {
			LOG_DBG("Database Hash matches");
			k_work_cancel_delayable(&hdev->gatt->gatt_sc.work);
			atomic_clear_bit(hdev->gatt->gatt_sc.flags, SC_RANGE_CHANGED);
			return;
		}

		LOG_HEXDUMP_DBG(hdev->gatt->db_hash.hash, sizeof(hdev->gatt->db_hash.hash), "New Hash: ");
			    ...
			    void *buf, uint16_t len, uint16_t offset)
{
	 ...
	 */
	(void)k_work_cancel_delayable_sync(&hdev->gatt->db_hash.work, &hdev->gatt->db_hash.sync);
	if (!atomic_test_bit(hdev->gatt->gatt_sc.flags, DB_HASH_VALID)) {
		db_hash_gen();
		if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
			set_all_change_unaware();
			db_hash_store();
	...
	return bt_gatt_attr_read(conn, attr, buf, len, offset, hdev->gatt->db_hash.hash,
				 sizeof(hdev->gatt->db_hash.hash));
}
...
static void gatt_insert(struct bt_gatt_service *svc, uint16_t last_handle)
{
	...
	if (last_handle == 0 || svc->attrs[0].handle > last_handle) {
		sys_slist_append(&svc->hdev->gatt->db, &svc->node);
		return;
	}

	/* DB shall always have its service in ascending order */
	SYS_SLIST_FOR_EACH_CONTAINER(&svc->hdev->gatt->db, tmp, node) {
		if (tmp->attrs[0].handle > svc->attrs[0].handle) {
			if (prev) {
				sys_slist_insert(&svc->hdev->gatt->db, &prev->node, &svc->node);
			} else {
				sys_slist_prepend(&svc->hdev->gatt->db, &svc->node);
			}
			return;
		}
...
static int gatt_register(struct bt_gatt_service *svc)
{
	...
	if (sys_slist_is_empty(&svc->hdev->gatt->db)) {
		handle = svc->hdev->gatt->last_static_handle;
		last_handle = 0;
		goto populate;
	}

	last = SYS_SLIST_PEEK_TAIL_CONTAINER(&svc->hdev->gatt->db, last, node);
...
{
#if defined(CONFIG_BT_GATT_SERVICE_CHANGED)
	k_work_reschedule(&hdev->gatt->gatt_sc.work, timeout);
#endif
}
			    ...
			    struct bt_gatt_indicate_params *params, uint8_t err)
{
	...
	atomic_clear_bit(hdev->gatt->gatt_sc.flags, SC_INDICATE_PENDING);

	/* Check if there is new change in the meantime */
	if (atomic_test_bit(hdev->gatt->gatt_sc.flags, SC_RANGE_CHANGED)) {
					       ...
					       const bt_addr_le_t *peer_addr)
{
	IF_ENABLED(CONFIG_BT_SETTINGS_DELAYED_STORE, ({
		...
		for (size_t i = 0; i < ARRAY_SIZE(hdev->gatt->gatt_delayed_store.peer_list); i++) {
			el = &hdev->gatt->gatt_delayed_store.peer_list[i];
			if (el->id == id &&
				       ...
				       enum delayed_store_flags flag)
{
	...
	if (bonded) {
		...
		k_work_reschedule(&hdev->gatt->gatt_delayed_store.work,
...
static void delayed_store(struct k_work *work)
{
	...
	struct bt_gatt_pool *gatt = CONTAINER_OF(store, struct bt_gatt_pool, gatt_delayed_store);
...
{
	if (atomic_test_and_set_bit(hdev->gatt->gatt_flags, GATT_SERVICE_INITIALIZED)) {
		return;
	}

	STRUCT_SECTION_FOREACH(bt_gatt_service_static, svc) {
		hdev->gatt->last_static_handle += svc->attr_count;
	}
}

void bt_gatt_init(struct bt_dev *hdev)
{
	hdev->gatt = &gatt_pool[hdev->dev_id];
	hdev->gatt->hdev = hdev;

	sys_slist_init(&hdev->gatt->callback_list);

#if defined(CONFIG_BT_GATT_NOTIFY_MULTIPLE)
	k_work_init(&hdev->gatt->nfy_mult_work, notify_mult_process);
#endif

	if (atomic_test_and_set_bit(hdev->gatt->gatt_flags, GATT_INITIALIZED)) {
		return;
	}
...
#if defined(CONFIG_BT_GATT_CACHING)
	k_work_init_delayable(&hdev->gatt->db_hash.work, db_hash_process);
	...
	if (IS_ENABLED(CONFIG_BT_LONG_WQ)) {
		bt_long_wq_schedule(&hdev->gatt->db_hash.work, DB_HASH_TIMEOUT);
	} else {
		k_work_schedule(&hdev->gatt->hdev->gatt->db_hash.work, DB_HASH_TIMEOUT);
	}
#endif /* CONFIG_BT_GATT_CACHING */

#if defined(CONFIG_BT_GATT_SERVICE_CHANGED)
	k_work_init_delayable(&hdev->gatt->gatt_sc.work, sc_process);
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		/* Make sure to not send SC indications until SC
		 * settings are loaded
		 */
		atomic_set_bit(hdev->gatt->gatt_sc.flags, SC_INDICATE_PENDING);
	}
#endif /* defined(CONFIG_BT_GATT_SERVICE_CHANGED) */
...
static void sc_indicate(struct bt_dev *hdev, uint16_t start, uint16_t end)
{
	...
	if (!atomic_test_and_set_bit(hdev->gatt->gatt_sc.flags, SC_RANGE_CHANGED)) {
		hdev->gatt->gatt_sc.start = start;
		hdev->gatt->gatt_sc.end = end;
		goto submit;
	}

	if (!update_range(&hdev->gatt->gatt_sc.start, &hdev->gatt->gatt_sc.end, start, end)) {
		return;
	}

submit:
	if (atomic_test_bit(hdev->gatt->gatt_sc.flags, SC_INDICATE_PENDING)) {
...
void bt_gatt_cb_register_mc(uint8_t dev_id, struct bt_gatt_cb *cb)
{
	...
	sys_slist_append(&hdev->gatt->callback_list, &cb->node);
}
...
static void db_changed(struct bt_dev *hdev)
{
	...
	atomic_clear_bit(hdev->gatt->gatt_sc.flags, DB_HASH_VALID);

	if (IS_ENABLED(CONFIG_BT_LONG_WQ)) {
		bt_long_wq_reschedule(&hdev->gatt->db_hash.work, DB_HASH_TIMEOUT);
	} else {
		k_work_reschedule(&hdev->gatt->db_hash.work, DB_HASH_TIMEOUT);
	}
...
{
	if (!sys_slist_find_and_remove(&svc->hdev->gatt->db, &svc->node)) {
		return -ENOENT;
	}
...
int bt_gatt_service_register_mc(uint8_t dev_id, struct bt_gatt_service *svc)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS) &&
	    atomic_test_bit(svc->hdev->gatt->gatt_flags, GATT_INITIALIZED) &&
	    !atomic_test_bit(svc->hdev->gatt->gatt_sc.flags, SC_LOAD)) {
	...
	if (!atomic_test_bit(svc->hdev->gatt->gatt_flags, GATT_INITIALIZED)) {
		k_sched_unlock();
		return 0;
	}
...
bool bt_gatt_service_is_registered(const struct bt_gatt_service *svc)
{
	...
	k_sched_lock();
	SYS_SLIST_FOR_EACH_NODE(&svc->hdev->gatt->db, node) {
		if (&svc->node == node) {
			registered = true;
			break;
		}
				    ...
				    bt_gatt_attr_func_t func, void *user_data)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&hdev->gatt->db, svc, node) {
			       ...
			       bt_gatt_attr_func_t func, void *user_data)
{
	...
	if (start_handle <= hdev->gatt->last_static_handle) {
		uint16_t handle = 1;
...
{
	struct bt_gatt_pool *gatt = CONTAINER_OF(work, struct bt_gatt_pool, nfy_mult_work);
	int i;
...
{
	int err = 0;
	struct net_buf **buf = &conn->hdev->gatt->nfy_mult[bt_conn_index(conn)];
			    ...
			    struct bt_gatt_notify_params *params)
{
	 ...
	 */
	k_work_schedule(&conn->hdev->gatt->nfy_mult_work,
			 ...
			 void *user_data)
{
	...
	if (IS_ENABLED(CONFIG_BT_GATT_SERVICE_CHANGED) && ccc == &sc_ccc) {
		for (i = 0; i < ARRAY_SIZE(data->hdev->gatt->sc_cfg); i++) {
			struct gatt_sc_cfg *cfg = &data->hdev->gatt->sc_cfg[i];
			struct bt_conn *conn;
...
static void sc_restore(struct bt_conn *conn)
{
	...
	hdev->gatt->sc_range[index][0] = sys_cpu_to_le16(cfg->data.start);
	hdev->gatt->sc_range[index][1] = sys_cpu_to_le16(cfg->data.end);

	hdev->gatt->sc_restore_params[index].attr = &_1_gatt_svc.attrs[2];
	hdev->gatt->sc_restore_params[index].func = sc_restore_rsp;
	hdev->gatt->sc_restore_params[index].data = &hdev->gatt->sc_range[index][0];
	hdev->gatt->sc_restore_params[index].len = sizeof(hdev->gatt->sc_range[index]);

	if (bt_gatt_indicate(conn, &hdev->gatt->sc_restore_params[index])) {
...
{
	for (int i = 0; i < ARRAY_SIZE(hdev->gatt->subscriptions); i++) {
		struct gatt_sub *sub = &hdev->gatt->subscriptions[i];
...
void bt_gatt_att_max_mtu_changed(struct bt_conn *conn, uint16_t tx, uint16_t rx)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&conn->hdev->gatt->callback_list, cb, node) {
					    ...
					    uint8_t id, const bt_addr_le_t *addr)
{
	if (IS_ENABLED(CONFIG_BT_GATT_CACHING)) {
		...
		for (i = 0; i < ARRAY_SIZE(hdev->gatt->cf_cfg); i++) {
			if (id == hdev->gatt->cf_cfg[i].id &&
			    bt_addr_le_eq(addr, &hdev->gatt->cf_cfg[i].peer)) {
				return &hdev->gatt->cf_cfg[i];
			}
		}
	}
...
{
	atomic_set_bit(hdev->gatt->gatt_sc.flags, SC_LOAD);
	atomic_clear_bit(hdev->gatt->gatt_sc.flags, SC_INDICATE_PENDING);
		       ...
		       settings_read_cb read_cb, void *cb_arg)
{
	...
	len = read_cb(cb_arg, hdev->gatt->db_hash.stored_hash, sizeof(hdev->gatt->db_hash.stored_hash));
	if (len < 0) {
		LOG_ERR("Failed to decode value (err %zd)", len);
		return len;
	}

	LOG_HEXDUMP_DBG(hdev->gatt->db_hash.stored_hash, sizeof(hdev->gatt->db_hash.stored_hash), "Stored Hash: ");
...
{
	atomic_set_bit(hdev->gatt->gatt_sc.flags, DB_HASH_LOAD);
