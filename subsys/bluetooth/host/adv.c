				 ...
				 struct bt_conn **out_conn)
{
	...
	if (!adv_is_directed(adv)) {
		...
		conn = bt_conn_add_le(adv->hdev, adv->id, BT_ADDR_LE_NONE);
		if (!conn) {
			return -ENOMEM;
		}
	...
	if (bt_conn_exists_le(adv->hdev, adv->id, &adv->target_addr)) {
		return -EINVAL;
	}

	conn = bt_conn_add_le(adv->hdev, adv->id, &adv->target_addr);
	if (!conn) {
		return -ENOMEM;
	}
	...
	if (adv->cb && adv->cb->scanned) {
		...
		if (bt_addr_le_is_resolved(&evt->addr)) {
			...
			bt_addr_le_copy(&id_addr,
					bt_lookup_id_addr(adv->hdev, adv->id, &evt->addr));
		}
