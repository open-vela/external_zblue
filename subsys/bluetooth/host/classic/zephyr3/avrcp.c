...
#include <zephyr/bluetooth/zephyr3/avrcp.h>
...
#include "host/classic/l2cap_br_internal.h"
...
#define AVRCP_KWORK(_work)                                                                         \
	CONTAINER_OF(CONTAINER_OF(_work, struct k_work_delayable, work), struct bt_avrcp_req,          \
		     timeout_work)
...
static int avrcp_send(struct bt_avrcp *session, struct net_buf *buf)
{
	...
	if ((hdr.cr == BT_AVRCP_CMD) &&
		...
		k_work_schedule(&session->req.timeout_work, AVRCP_TIMEOUT);
	}
	return 0;
}
/* Timeout handler */
...
{
	struct bt_avrcp_req *req = AVRCP_KWORK(work);
	struct bt_avrcp *session = CONTAINER_OF(req, struct bt_avrcp, req);
	LOG_DBG("Failed subunit_type:%d, opcode:%d tid:%d", req->subunit_type, req->opcode,req->tid);
	if (req->timeout_func) {
...
static void bt_avrcp_l2cap_connected(struct bt_l2cap_chan *chan)
{
	...
	k_work_init_delayable(&session->req.timeout_work, avrcp_timeout);
								...
								struct net_buf *buf)
{
	...
	if (info->hdr.ctype != BT_AVRCP_CTYPE_ACCEPTED) {
		LOG_ERR("Responed ctyep:%d not accepted", info->hdr.ctype);
		return;
	}
								...
								struct net_buf *buf)
{
	...
	LOG_DBG("len:0x%x, pos:0x%x, status:%d", len, pos, rsp->status);
								...
								struct net_buf *buf)
{
	...
	if (rsp->info.hdr.ctype == BT_AVRCP_CTYPE_CHANGED_STABLE) {
		if (rsp->event_id == BT_AVRCP_EVENT_PLAYBACK_STATUS_CHANGED) {
			...
			LOG_INF("Notify volume change, value %d\n", rsp->status);
		}
		avrcp_state_sm_work(&session->req);
								...
								struct net_buf *buf)
{
	...
	while(total_len > 0 && num > 0){
		...
		if (total_len < sizeof(struct Attribute)) {
			LOG_ERR("total_len %d, header %d.", total_len, sizeof(struct Attribute));
			return;
		}
		...
		if (total_len < sizeof(struct Attribute) + attribute->len) {
			LOG_ERR("total_len %d, attrlen %d.", total_len, attribute->len);
			return;
		}
...
int bt_avrcp_ctrl_l2cap_accept(struct bt_conn *conn, struct bt_l2cap_server *server, 
...
int bt_avrcp_init(void)
{
	...
	if (err < 0) {
		LOG_ERR("AVRCP L2CAP Registration failed %d", err);
	}
	return err;
}
