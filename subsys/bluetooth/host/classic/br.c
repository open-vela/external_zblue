...
static void report_discovery_results(struct bt_dev *hdev)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&hdev->discovery_cbs, listener, next, node) {
		if (listener->timeout) {
			listener->timeout(hdev->dev_id, hdev->discovery_results, hdev->discovery_results_count);
		}
	}
...
void bt_hci_inquiry_result_with_rssi(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	while (num_reports--) {
		...
		SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&hdev->discovery_cbs, listener, next, node) {
			if (listener->recv) {
				listener->recv(hdev->dev_id, result);
			}
		}
	}
}
...
int bt_br_unpair_mc(uint8_t dev_id, bt_addr_t *bdaddr)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&hdev->bt_auth_info_cbs, listener,
		...
		if (listener->bond_deleted) {
			listener->bond_deleted(hdev->dev_id, 0, &addr);
		}
	}
