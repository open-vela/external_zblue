...
static struct bt_sdp_record *bt_sdp_foreach_svc(struct bt_dev *hdev, bt_sdp_svc_func_t func,
						void *user_data)
{
	struct bt_sdp_record *rec = hdev->sdp->db;
			      ...
			      struct bt_sdp_record **matching_recs)
{
	...
	while (uuid_list_size) {
		 ...
		 */
		for (rec_idx = 0U; rec_idx < hdev->sdp->num_services; rec_idx++) {
				...
				uint16_t tid)
{
	...
	for (idx = 0U; idx < hdev->sdp->num_services; idx++) {
	...
	for (; cont_state < hdev->sdp->num_services; cont_state++) {
				    ...
				    uint16_t tid)
{
	...
	for (; next_svc < hdev->sdp->num_services; next_svc++) {
			 ...
			 struct bt_l2cap_chan **chan)
{
	...
	for (i = 0; i < ARRAY_SIZE(hdev->sdp->bt_sdp_pool); i++) {
		struct bt_sdp *sdp = &hdev->sdp->bt_sdp_pool[i];
...
void bt_sdp_init(struct bt_dev *hdev)
{
	...
	hdev->sdp = &sdp_ctx[hdev->dev_id];
	memset(hdev->sdp, 0, sizeof(*hdev->sdp));
...
int bt_sdp_register_service_mc(uint8_t dev_id, struct bt_sdp_record *service)
{
	...
	if (hdev->sdp->num_services == BT_SDP_MAX_SERVICES) {
		LOG_ERR("Reached max allowed registrations");
		return -ENOMEM;
	}

	if (hdev->sdp->db) {
		handle = hdev->sdp->db->handle + 1;
	}

	service->next = hdev->sdp->db;
	service->index = hdev->sdp->num_services++;
...
static struct bt_sdp_client *sdp_client_new_session(struct bt_conn *conn)
{
	...
	for (i = 0; i < ARRAY_SIZE(hdev->sdp->bt_sdp_client_pool); i++) {
		struct bt_sdp_client *session = &hdev->sdp->bt_sdp_client_pool[i];
		int err;
...
static struct bt_sdp_client *sdp_client_get_session(struct bt_conn *conn)
{
	...
	for (i = 0; i < ARRAY_SIZE(hdev->sdp->bt_sdp_client_pool); i++) {
		if (hdev->sdp->bt_sdp_client_pool[i].chan.chan.conn == conn) {
			return &hdev->sdp->bt_sdp_client_pool[i];
		}
	}
