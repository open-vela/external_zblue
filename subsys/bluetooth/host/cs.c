...
int bt_le_cs_read_remote_supported_capabilities(struct bt_conn *conn)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_READ_REMOTE_SUPPORTED_CAPABILITIES, buf, NULL);
}

void bt_hci_le_cs_read_remote_supported_capabilities_complete(struct bt_dev *hdev, struct net_buf *buf)
{
	struct bt_conn *conn;
				  ...
				  const struct bt_le_cs_set_default_settings_param *params)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_SET_DEFAULT_SETTINGS, buf, NULL);
}
...
int bt_le_cs_read_remote_fae_table(struct bt_conn *conn)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_READ_REMOTE_FAE_TABLE, buf, NULL);
}
...
int bt_le_cs_start_test(const struct bt_le_cs_test_param *params)
{
	...
	return bt_hci_cmd_send_sync(&bt_dev, BT_HCI_OP_LE_CS_TEST, buf, NULL);
}
#endif /* CONFIG_BT_CHANNEL_SOUNDING_TEST */
			   ...
			   enum bt_le_cs_create_config_context context)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_CREATE_CONFIG, buf, NULL);
}
...
int bt_le_cs_remove_config(struct bt_conn *conn, uint8_t config_id)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_REMOVE_CONFIG, buf, NULL);
}
...
int bt_le_cs_security_enable(struct bt_conn *conn)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_SECURITY_ENABLE, buf, NULL);
}
			      ...
			      const struct bt_le_cs_procedure_enable_param *params)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_PROCEDURE_ENABLE, buf, NULL);
}
				      ...
				      const struct bt_le_cs_set_procedure_parameters_param *params)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_SET_PROCEDURE_PARAMETERS, buf, NULL);
}
...
int bt_le_cs_set_channel_classification(uint8_t channel_classification[10])
{
	...
	return bt_hci_cmd_send_sync(&bt_dev, BT_HCI_OP_LE_CS_SET_CHANNEL_CLASSIFICATION, buf, NULL);
}
...
int bt_le_cs_read_local_supported_capabilities(struct bt_conn_le_cs_capabilities *ret)
{
	...
	int err =
		bt_hci_cmd_send_sync(&bt_dev, BT_HCI_OP_LE_CS_READ_LOCAL_SUPPORTED_CAPABILITIES, NULL, &rsp);
	...
	struct bt_conn *conn, const struct bt_conn_le_cs_capabilities *params)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_WRITE_CACHED_REMOTE_SUPPORTED_CAPABILITIES, buf,
				    NULL);
}
...
int bt_le_cs_write_cached_remote_fae_table(struct bt_conn *conn, int8_t remote_fae_table[72])
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CS_WRITE_CACHED_REMOTE_FAE_TABLE, buf, NULL);
}
...
int bt_le_cs_stop_test(void)
{
	...
	return bt_hci_cmd_send_sync(&bt_dev, BT_HCI_OP_LE_CS_TEST_END, buf, NULL);
}
