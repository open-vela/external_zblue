...
struct bt_scan_pool {
	...
	struct bt_le_per_adv_sync per_adv_sync_pool[CONFIG_BT_PER_ADV_SYNC_MAX];
	sys_slist_t pa_sync_cbs;
#endif

#if defined(CONFIG_BT_PER_ADV_SYNC_TRANSFER_RECEIVER)
	uint8_t conn_past_modes[CONFIG_BT_MAX_CONN];
	uint8_t default_past_mode;
#endif
#endif
} scan_pool[CONFIG_BT_NUM_CTLRS];
...
static void init_reassembling_advertiser(struct bt_dev *hdev, const bt_addr_le_t *addr, uint8_t sid)
{
	bt_addr_le_copy(&hdev->scan->reassembling_advertiser.addr, addr);
	hdev->scan->reassembling_advertiser.sid = sid;
	hdev->scan->reassembling_advertiser.state = FRAG_ADV_REASSEMBLING;
}

static void reset_reassembling_advertiser(struct bt_dev *hdev)
{
	net_buf_simple_reset(&ext_scan_buf);
	hdev->scan->reassembling_advertiser.state = FRAG_ADV_INACTIVE;
}

#if defined(CONFIG_BT_PER_ADV_SYNC)
static struct bt_le_per_adv_sync *get_pending_per_adv_sync(struct bt_dev *hdev);
#endif /* defined(CONFIG_BT_PER_ADV_SYNC) */
#endif /* defined(CONFIG_BT_EXT_ADV) */

void bt_scan_softreset(struct bt_dev *hdev)
{
	hdev->scan->scan_dev_found_cb = NULL;
#if defined(CONFIG_BT_EXT_ADV)
...
{
	hdev->scan = &scan_pool[hdev->dev_id];
	hdev->scan->hdev = hdev;
	sys_slist_init(&hdev->scan->scan_cbs);
#if defined(CONFIG_BT_PER_ADV_SYNC)
	memset(&hdev->scan->per_adv_sync_pool, 0x0, sizeof(hdev->scan->per_adv_sync_pool));
	sys_slist_init(&hdev->scan->pa_sync_cbs);
#endif /* defined(CONFIG_BT_PER_ADV_SYNC) */

	memset(&hdev->scan->scan_state, 0x0, sizeof(hdev->scan->scan_state));
	k_mutex_init(&hdev->scan->scan_state.scan_update_mutex);
	k_mutex_init(&hdev->scan->scan_state.scan_explicit_params_mutex);
	bt_scan_softreset(hdev);
}

static int cmd_le_set_ext_scan_enable(struct bt_dev *hdev, bool enable,
			  bool filter_duplicates, uint16_t duration)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_SET_EXT_SCAN_ENABLE, buf, NULL);
	if (err) {
		return err;
	}
...
static int cmd_le_set_scan_enable_legacy(struct bt_dev *hdev, bool enable, bool filter_duplicates)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_SET_SCAN_ENABLE, buf, NULL);
	if (err) {
		return err;
	}
...
{
	return cmd_le_set_scan_enable(hdev, enable, hdev->scan->scan_state.used_scan_param.options &
...
static int start_le_scan_ext(struct bt_dev *hdev, struct bt_le_scan_param *scan_param)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_SET_EXT_SCAN_PARAM, buf, NULL);
	if (err) {
		return err;
	}
...
static int start_le_scan_legacy(struct bt_dev *hdev, struct bt_le_scan_param *param)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_SET_SCAN_PARAM, buf, NULL);
	if (err) {
		return err;
	}
...
bool bt_le_scan_active_scanner_running(struct bt_dev *hdev)
{
	return atomic_test_bit(hdev->flags, BT_DEV_SCANNING) &&
	       hdev->scan->scan_state.used_scan_param.type == BT_LE_SCAN_TYPE_ACTIVE;
}
...
static void select_scan_params(struct bt_dev *hdev, struct bt_le_scan_param *scan_param)
{
	...
	if (atomic_test_bit(hdev->scan->scan_state.scan_flags, BT_LE_SCAN_USER_EXPLICIT_SCAN)) {
		memcpy(scan_param, &hdev->scan->scan_state.explicit_scan_param, sizeof(*scan_param));
	}
	...
	/* 2. Priority: reuse parameters from initiator */
		...
		*scan_param = (struct bt_le_scan_param){
			...
			.timeout = 0,
			.interval_coded = hdev->create_param.interval_coded,
			.window_coded = hdev->create_param.window_coded,
		};
	}
	/* 3. Priority: choose custom parameters */
		...
		*scan_param = (struct bt_le_scan_param){
		...
		if (BT_FEAT_LE_PHY_CODED(hdev->le.features)) {
			scan_param->options |= BT_LE_SCAN_OPT_CODED;
		}

		if (atomic_test_bit(hdev->scan->scan_state.scan_flags, BT_LE_SCAN_USER_PER_SYNC) ||
		    atomic_test_bit(hdev->scan->scan_state.scan_flags, BT_LE_SCAN_USER_CONN)) {
...
static bool is_already_using_same_params(struct bt_dev *hdev, struct bt_le_scan_param *scan_param)
{
	return !memcmp(scan_param, &hdev->scan->scan_state.used_scan_param, sizeof(*scan_param));
}

static enum scan_action get_scan_action(struct bt_dev *hdev,
				        struct bt_le_scan_param *scan_param)
{
	bool is_scanning = atomic_test_bit(hdev->flags, BT_DEV_SCANNING);

	/* Check if there is reason to have the scanner running */
	if (atomic_get(hdev->scan->scan_state.scan_flags) != 0) {
		if (is_scanning) {
			if (is_already_using_same_params(hdev, scan_param)) {
...
static int scan_update(struct bt_dev *hdev)
{
	int32_t err;
	...
	err = k_mutex_lock(&hdev->scan->scan_state.scan_update_mutex, K_NO_WAIT);

	if (err) {
		return err;
	}

	select_scan_params(hdev, &scan_param);

	enum scan_action action = get_scan_action(hdev, &scan_param);
	...
	case SCAN_ACTION_STOP:
		...
		memset(&hdev->scan->scan_state.used_scan_param, 0x0,
		       sizeof(hdev->scan->scan_state.used_scan_param));
		break;
	...
	case SCAN_ACTION_START:
		...
		memcpy(&hdev->scan->scan_state.used_scan_param, &scan_param, sizeof(scan_param));
		break;
	}

	k_mutex_unlock(&hdev->scan->scan_state.scan_update_mutex);

	return err;
}

static int scan_check_if_state_allowed(struct bt_dev *hdev, enum bt_le_scan_user flag)
{
	/* check if state is already set */
	if (atomic_test_bit(hdev->scan->scan_state.scan_flags, flag)) {
		return -EALREADY;
	}

	if (flag == BT_LE_SCAN_USER_EXPLICIT_SCAN && !BT_LE_STATES_SCAN_INIT(hdev->scan->le.states) &&
...
int bt_le_scan_user_add(struct bt_dev *hdev, enum bt_le_scan_user flag)
{
	...
	err = scan_check_if_state_allowed(hdev, flag);
	if (err) {
		return err;
	}

	atomic_set_bit(hdev->scan->scan_state.scan_flags, flag);

	err = scan_update(hdev);
	if (err) {
		atomic_clear_bit(hdev->scan->scan_state.scan_flags, flag);
	}
			       ...
			       const bt_addr_le_t *addr, uint8_t adv_props)
{
	 ...
	 */
	if (!BT_LE_STATES_SCAN_INIT(hdev->scan->le.states) &&
	    atomic_test_bit(hdev->scan->scan_state.scan_flags, BT_LE_SCAN_USER_EXPLICIT_SCAN)) {
		return;
	}
	...
	conn = bt_conn_lookup_state_le(hdev, BT_ID_DEFAULT, id_addr,
	 ...
	 */
	err = bt_le_scan_user_remove(hdev, BT_LE_SCAN_USER_CONN);
	if (err) {
...
static void le_adv_recv(struct bt_dev *hdev, bt_addr_le_t *addr,
			struct bt_le_scan_recv_info *info, struct net_buf_simple *buf, uint16_t len)
{
	...
	if (hdev->scan->scan_dev_found_cb) {
		net_buf_simple_save(buf, &state);

		buf->len = len;
		hdev->scan->scan_dev_found_cb(&id_addr, info->rssi, info->adv_type, buf);
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&hdev->scan->scan_cbs, listener, next, node) {
		if (listener->recv) {
			net_buf_simple_save(buf, &state);
...
void bt_hci_le_adv_ext_report(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	while (num_reports--) {
		...
		if (!atomic_test_bit(hdev->scan->scan_state.scan_flags, BT_LE_SCAN_USER_EXPLICIT_SCAN)) {
			...
			if (hdev->scan->reassembling_advertiser.state != FRAG_ADV_INACTIVE) {
				reset_reassembling_advertiser(hdev);
			}
		...
		if (evt->length > buf->len) {
			 ...
			 * data.
			 */
			hdev->scan->reassembling_advertiser.state = FRAG_ADV_DISCARDING;
		...
		if (evt_type & BT_HCI_LE_ADV_EVT_TYPE_LEGACY) {
			...
			le_adv_recv(hdev, &evt->addr, &scan_info, &buf->b, evt->length);
			goto cont;
		}

		is_new_advertiser = hdev->scan->reassembling_advertiser.state == FRAG_ADV_INACTIVE ||
				    !fragmented_advertisers_equal(&hdev->scan->reassembling_advertiser,
								  &evt->addr, evt->sid);
		...
		if (is_new_advertiser && hdev->scan->reassembling_advertiser.state == FRAG_ADV_REASSEMBLING) {
		...
		if (is_new_advertiser) {
			 ...
			 */
			__ASSERT_NO_MSG(hdev->scan->reassembling_advertiser.state == FRAG_ADV_INACTIVE);
			init_reassembling_advertiser(hdev, &evt->addr, evt->sid);
		}
		...
		if (hdev->scan->reassembling_advertiser.state == FRAG_ADV_DISCARDING) {
			if (!more_to_come) {
		...
		create_ext_adv_info(evt, &scan_info);
		le_adv_recv(hdev, &evt->addr, &scan_info, &ext_scan_buf, ext_scan_buf.len);
...
{
	per_adv_sync->hdev = NULL;
	atomic_clear(per_adv_sync->flags);
}

static struct bt_le_per_adv_sync *per_adv_sync_new(struct bt_dev *hdev)
{
	struct bt_le_per_adv_sync *per_adv_sync = NULL;

	for (int i = 0; i < ARRAY_SIZE(hdev->scan->per_adv_sync_pool); i++) {
		if (!atomic_test_bit(hdev->scan->per_adv_sync_pool[i].flags,
				     BT_PER_ADV_SYNC_CREATED)) {
			per_adv_sync = &hdev->scan->per_adv_sync_pool[i];
			break;
		}
	}
...
static struct bt_le_per_adv_sync *get_pending_per_adv_sync(struct bt_dev *hdev)
{
	for (int i = 0; i < ARRAY_SIZE(hdev->scan->per_adv_sync_pool); i++) {
		if (atomic_test_bit(hdev->scan->per_adv_sync_pool[i].flags,
			...
			return &hdev->scan->per_adv_sync_pool[i];
		}
	}
...
{
	for (size_t i = 0; i < ARRAY_SIZE(hdev->scan->per_adv_sync_pool); i++) {
		per_adv_sync_delete(&hdev->scan->per_adv_sync_pool[i]);
	}
}

struct bt_le_per_adv_sync *bt_hci_per_adv_sync_lookup_handle(struct bt_dev *hdev, uint16_t handle)
{
	for (int i = 0; i < ARRAY_SIZE(hdev->scan->per_adv_sync_pool); i++) {
		if (hdev->scan->per_adv_sync_pool[i].handle == handle &&
		    atomic_test_bit(hdev->scan->per_adv_sync_pool[i].flags,
				   ...
				   const struct bt_le_per_adv_sync_recv_info *info)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&per_adv_sync->hdev->scan->pa_sync_cbs, listener, node) {
		if (listener->recv) {
			net_buf_simple_save(buf, &state);
...
static void bt_hci_le_per_adv_report_common(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	per_adv_sync = bt_hci_per_adv_sync_lookup_handle(hdev, sys_le16_to_cpu(evt->handle));
...
static int per_adv_sync_terminate(struct bt_dev *hdev, uint16_t handle)
{
	...
	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_PER_ADV_TERMINATE_SYNC, buf,
				    NULL);
}
...
static void bt_hci_le_per_adv_sync_established_common(struct bt_dev *hdev,
				  struct net_buf *buf)
{
#if defined(CONFIG_BT_PER_ADV_SYNC_RSP)
	...
	pending_per_adv_sync = get_pending_per_adv_sync(hdev);

	if (pending_per_adv_sync) {
		atomic_clear_bit(pending_per_adv_sync->flags, BT_PER_ADV_SYNC_SYNCING);
		err = bt_le_scan_user_remove(hdev, BT_LE_SCAN_USER_PER_SYNC);
	...
	if (!pending_per_adv_sync ||
		...
		per_adv_sync_terminate(hdev, sys_le16_to_cpu(evt->handle));
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&hdev->scan->pa_sync_cbs, listener, node) {
		if (listener->synced) {
				...
				struct bt_le_per_adv_sync_subevent_params *params)
{
	...
	return bt_hci_cmd_send_sync(per_adv_sync->hdev, BT_HCI_OP_LE_SET_PER_ADV_SYNC_SUBEVENT, buf, NULL);
}
				    ...
				    const struct net_buf_simple *data)
{
	...
	return bt_hci_cmd_send_sync(per_adv_sync->hdev, BT_HCI_OP_LE_SET_PER_ADV_RESPONSE_DATA, buf, NULL);
}
#endif /* CONFIG_BT_PER_ADV_SYNC_RSP */
...
static void past_disconnected_cb(struct bt_conn *conn, uint8_t reason)
{
	 ...
	 */
	conn->hdev->scan->conn_past_modes[bt_conn_index(conn)] = BT_HCI_LE_PAST_MODE_NO_SYNC;
}
...
static void bt_hci_le_past_received_common(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	if (!sync_info.conn) {
		...
		per_adv_sync_terminate(hdev, sys_le16_to_cpu(evt->sync_handle));
		return;
	}

	per_adv_sync = per_adv_sync_new(hdev);
	if (!per_adv_sync) {
	...
	const uint8_t mode = hdev->scan->conn_past_modes[bt_conn_index(sync_info.conn)];
...
void bt_hci_le_past_received(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	bt_hci_le_past_received_common(hdev, buf);
}
...
void bt_hci_le_biginfo_adv_report(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	per_adv_sync = bt_hci_per_adv_sync_lookup_handle(hdev, sys_le16_to_cpu(evt->sync_handle));
...
void bt_hci_le_adv_report(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	while (num_reports--) {
		...
		le_adv_recv(hdev, &evt->addr, &adv_info, &buf->b, evt->length);
...
int bt_le_scan_start_mc(uint8_t dev_id, const struct bt_le_scan_param *param, bt_le_scan_cb_t cb)
{
	 ...
	 */
	err = k_mutex_lock(&hdev->scan->scan_state.scan_explicit_params_mutex, K_NO_WAIT);

	if (err) {
		return err;
	}

	err = scan_check_if_state_allowed(hdev, BT_LE_SCAN_USER_EXPLICIT_SCAN);

	if (err) {
		k_mutex_unlock(&hdev->scan->scan_state.scan_explicit_params_mutex);
		return err;
	}

	/* store the parameters that were used to start the scanner */
	memcpy(&hdev->scan->scan_state.explicit_scan_param, param,
	       sizeof(hdev->scan->scan_state.explicit_scan_param));

	hdev->scan->scan_dev_found_cb = cb;
...
int bt_le_scan_cb_register_mc(uint8_t dev_id, struct bt_le_scan_cb *cb)
{
	...
	if (sys_slist_find(&hdev->scan->scan_cbs, &cb->node, NULL)) {
		return -EEXIST;
	}

	sys_slist_append(&hdev->scan->scan_cbs, &cb->node);
...
void bt_le_scan_cb_unregister_mc(uint8_t dev_id, struct bt_le_scan_cb *cb)
{
	...
	sys_slist_find_and_remove(&hdev->scan->scan_cbs, &cb->node);
}
...
uint8_t bt_le_per_adv_sync_get_index(struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	ptrdiff_t index = per_adv_sync - hdev->scan->per_adv_sync_pool;

	__ASSERT(index >= 0 && ARRAY_SIZE(hdev->scan->per_adv_sync_pool) > index,
...
struct bt_le_per_adv_sync *bt_le_per_adv_sync_lookup_index_mc(uint8_t dev_id, uint8_t index)
{
	...
	if (index >= ARRAY_SIZE(hdev->scan->per_adv_sync_pool)) {
		return NULL;
	}

	return &hdev->scan->per_adv_sync_pool[index];
}
							  ...
							  uint8_t sid)
{
	...
	for (int i = 0; i < ARRAY_SIZE(per_adv_sync_pool); i++) {
		struct bt_le_per_adv_sync *sync = &hdev->scan->per_adv_sync_pool[i];
			      ...
			      struct bt_le_per_adv_sync **out_sync)
{
	...
	if (!BT_FEAT_LE_EXT_PER_ADV(hdev->le.features)) {
		return -ENOTSUP;
	}

	if (get_pending_per_adv_sync(hdev)) {
		return -EBUSY;
	}
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_PER_ADV_CREATE_SYNC, buf, NULL);
	if (err) {
		per_adv_sync_delete(per_adv_sync);
		return err;
	}
	 ...
	 */
	err = bt_le_scan_user_add(hdev, BT_LE_SCAN_USER_PER_SYNC);
	if (err) {
		int per_sync_remove_err = bt_le_scan_user_remove(hdev, BT_LE_SCAN_USER_PER_SYNC);
	...
	struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	if (get_pending_per_adv_sync(per_adv_sync->hdev) != per_adv_sync) {
		return -EINVAL;
	}
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_PER_ADV_CREATE_SYNC_CANCEL, buf,
				   NULL);
	if (err) {
		return err;
	}
...
static int bt_le_per_adv_sync_terminate(struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	err = per_adv_sync_terminate(per_adv_sync->hdev, per_adv_sync->handle);
...
int bt_le_per_adv_sync_delete(struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	} else if (get_pending_per_adv_sync(hdev) == per_adv_sync) {
...
int bt_le_per_adv_sync_cb_register_mc(uint8_t dev_id, struct bt_le_per_adv_sync_cb *cb)
{
	...
	if (sys_slist_find(&hdev->scan->pa_sync_cbs, &cb->node, NULL)) {
		return -EEXIST;
	}

	sys_slist_append(&hdev->scan->pa_sync_cbs, &cb->node);
	...
	struct bt_le_per_adv_sync *per_adv_sync, bool enable)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_SET_PER_ADV_RECV_ENABLE,
				   buf, NULL);
				...
				uint16_t service_data)
{
	...
	if (!BT_FEAT_LE_EXT_PER_ADV(hdev->le.features)) {
		return -ENOTSUP;
	} else if (!BT_FEAT_LE_PAST_SEND(hdev->le.features)) {
		return -ENOTSUP;
	}
	...
	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_PER_ADV_SYNC_TRANSFER, buf,
				    NULL);
}
			  ...
			  uint16_t skip, uint16_t timeout, uint8_t cte_type)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_PAST_PARAM, buf, NULL);
}

static int default_past_param_set(struct bt_dev *hdev, uint8_t mode, uint16_t skip,
				  uint16_t timeout, uint8_t cte_type)
{
	...
	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_DEFAULT_PAST_PARAM, buf, NULL);
}
	...
	const struct bt_le_per_adv_sync_transfer_param *param)
{
	...
	if (!BT_FEAT_LE_EXT_PER_ADV(hdev->le.features)) {
		return -ENOTSUP;
	} else if (!BT_FEAT_LE_PAST_RECV(hdev->le.features)) {
		return -ENOTSUP;
	}
	...
	if (conn) {
		...
		const uint8_t old_mode = hdev->scan->conn_past_modes[conn_idx];

		hdev->scan->conn_past_modes[conn_idx] = mode;

		err = past_param_set(conn, mode, param->skip, param->timeout, cte_type);
		if (err != 0) {
			/* Restore old mode */
			hdev->scan->conn_past_modes[conn_idx] = old_mode;
		}
	} else {
		const uint8_t old_mode = hdev->scan->default_past_mode;

		hdev->scan->default_past_mode = mode;

		err = default_past_param_set(hdev, mode, param->skip, param->timeout, cte_type);
		if (err != 0) {
			/* Restore old mode */
			hdev->scan->default_past_mode = old_mode;
		}
	}
...
int bt_le_per_adv_sync_transfer_unsubscribe(const struct bt_conn *conn)
{
	...
	if (conn) {
		...
		hdev->scan->conn_past_modes[conn_idx] = BT_HCI_LE_PAST_MODE_NO_SYNC;
		...
		hdev->scan->default_past_mode = BT_HCI_LE_PAST_MODE_NO_SYNC;
		err = default_past_param_set(hdev, BT_HCI_LE_PAST_MODE_NO_SYNC, 0, 0x0a, 0);
		if (err != 0) {
			/* Restore old mode */
			hdev->scan->default_past_mode = old_mode;
		}
...
int bt_le_per_adv_list_add_mc(uint8_t dev_id, const bt_addr_le_t *addr, uint8_t sid)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_ADD_DEV_TO_PER_ADV_LIST, buf,
				   NULL);
	if (err) {
...
int bt_le_per_adv_list_remove_mc(uint8_t dev_id, const bt_addr_le_t *addr, uint8_t sid)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_REM_DEV_FROM_PER_ADV_LIST, buf,
				   NULL);
	if (err) {
...
int bt_le_per_adv_list_clear_mc(uint8_t dev_id)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_CLEAR_PER_ADV_LIST, NULL, NULL);
	if (err) {
...
{
	return atomic_test_bit(hdev->scan->scan_state.scan_flags, BT_LE_SCAN_USER_EXPLICIT_SCAN);
}
