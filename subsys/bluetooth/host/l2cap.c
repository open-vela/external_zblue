...
struct bt_l2cap_server *bt_l2cap_server_lookup_psm(struct bt_dev *hdev, uint16_t psm)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&hdev->l2cap->servers, server, node) {
		if (server->psm == psm) {
			return server;
		}
	}
...
int bt_l2cap_server_register_mc(uint8_t dev_id, struct bt_l2cap_server *server)
{
	...
	sys_slist_append(&hdev->l2cap->servers, &server->node);
...
callback:
	if (hdev->l2cap->ecred_cb && ecred_cb->ecred_conn_req) {
		hdev->l2cap->ecred_cb->ecred_conn_req(conn, result, psm);
	}
	if (rsp_queued) {
			      ...
			      struct net_buf *buf)
{
	...
	if (hdev->l2cap->ecred_cb && hdev->l2cap->ecred_cb->ecred_conn_rsp) {
		hdev->l2cap->ecred_cb->ecred_conn_rsp(conn, result, attempted, succeeded, psm);
	}
}
...
static int l2cap_accept(struct bt_conn *conn, struct bt_l2cap_chan **chan)
{
	...
	for (i = 0; i < ARRAY_SIZE(hdev->l2cap->bt_l2cap_pool); i++) {
		struct bt_l2cap *l2cap = &hdev->l2cap->bt_l2cap_pool[i];
...
{
	hdev->l2cap = &l2cap_pool[hdev->dev_id];

	sys_slist_init(&hdev->l2cap->servers);
	hdev->l2cap->ident = 0;
