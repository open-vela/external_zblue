...
typedef enum __packed {
	BT_AVRCP_DIRECTION_UP   = 0x00,
	BT_AVRCP_DIRECTION_DOWN = 0x01,
} bt_avrcp_direction_t;
...
typedef enum __packed {
	BT_AVRCP_MEDIA_TYPE_AUDIO = 0x00,
...
struct bt_avrcp_get_folder_items_cmd {
	...
	uint32_t attr_ids[];   /**< Attribute IDs list*/
} __packed;
 ...
 */
typedef enum __packed {
	BT_AVRCP_ITEM_TYPE_MEDIA_PLAYER  = 0x01U,
	BT_AVRCP_ITEM_TYPE_FOLDER        = 0x02U,
	BT_AVRCP_ITEM_TYPE_MEDIA_ELEMENT = 0x03U,
...
struct bt_avrcp_media_player_item {
	...
	uint8_t  play_status;         /**< bt_avrcp_playback_status_t */
	uint8_t  feature_bitmask[16]; /**< 128-bit Feature bitmask, octet0..15 */
	uint16_t charset_id;          /**< Displayable name charset (e.g. UTF-8) */
 ...
 *
 * The @p name_attr field contains:
 * - struct media_element_item_name (name_len + name[]).
...
struct bt_avrcp_media_element_item {
	...
	uint16_t charset_id;   /**< Character set for the name. */
	uint8_t  name_attr[];  /**< Flexible array for name and attributes. */
} __packed;
...
struct bt_avrcp_change_path_cmd {
	uint16_t uid_counter;       /**< UID counter */
	uint8_t  direction;         /**< bt_avrcp_direction_t (0=Up, 1=Down) */
...
struct bt_avrcp_search_cmd {
	uint16_t charset_id;        /**< e.g. BT_AVRCP_CHARSET_UTF8 */
...
struct bt_avrcp_ct_cb {
	 ...
	 *             parameters as defined by AVRCP PDU GET_FOLDER_ITEMS. The
	 *             application should parse this payload per the AVRCP specification.
	 ...
	 *             parameters as defined by AVRCP PDU GET_ITEM_ATTRIBUTES. The
	 *             application should parse this payload per the AVRCP specification.
	 *             Note that the data is encoded in big-endian format.
	 */
	 ...
	 *             response parameters as defined by AVRCP PDU
	 *             GET_TOTAL_NUMBER_OF_ITEMS (e.g. Status, UIDCounter, NumItems).
	 *             The application should parse this payload per the AVRCP
	 *             specification. Note that the data is encoded in big-endian format.
	 */
	 ...
	 *  @param ct  AVRCP CT connection object.
	 *  @param tid The transaction label of the response.
	 *  @param buf The response buffer containing the Search response parameters as
	 *             defined by AVRCP PDU SEARCH (e.g. Status, UIDCounter, NumItems).
	 *             The application should parse this payload per the AVRCP
	 *             specification. Note that the data is encoded in big-endian format.
	 */
 ...
 *             parameters (Scope, StartItem, EndItem, AttributeCount/IDs etc.)
 *             encoded in big-endian format as defined by the AVRCP specification.
 *
 *  @return 0 in case of success or error code in case of error.
 */
 ...
 *  @param ct  The AVRCP CT instance.
 *  @param tid The transaction label of the response, valid from 0 to 15.
 *  @param buf The command buffer containing the Change Path command parameters
 *             (UIDCounter, Direction, FolderUID) encoded in big-endian format as
 *             defined by the AVRCP specification.
 *
 *  @return 0 in case of success or error code in case of error.
 */
 ...
 *             parameters (Scope, UID, UIDCounter, AttributeCount/IDs etc.)
 *             encoded in big-endian format as defined by the AVRCP specification.
 *
 *  @return 0 in case of success or error code in case of error.
 */
 ...
 *  @param ct  The AVRCP CT instance.
 *  @param tid The transaction label of the response, valid from 0 to 15.
 *  @param buf The command buffer containing the Search command parameters
 *             (CharsetID, StringLength, String) encoded in big-endian format as
 *             defined by the AVRCP specification. The search string is not
 *             NUL-terminated; its length is given by StringLength.
 *
 *  @return 0 in case of success or error code in case of error.
 */
 ...
 *  @param tid The transaction label of the response, valid from 0 to 15.
 *  @param buf The response buffer containing the Get Folder Items response
 *             parameters encoded in big-endian format as defined by the AVRCP
 *             specification.
 *
 *  @return 0 in case of success or error code in case of error.
 */
 ...
 *  @param tid The transaction label of the response, valid from 0 to 15.
 *  @param buf The response buffer containing the Get Total Number Of Items
 *             response parameters (Status, UIDCounter, NumItems) encoded in
 *             big-endian format as defined by the AVRCP specification.
 *
 *  @return 0 in case of success or error code in case of error.
 */
 ...
 *             (Status, UIDCounter, NumItems) encoded in big-endian format as
 *             defined by the AVRCP specification.
 *
 *  @return 0 in case of success or error code in case of error.
 */
